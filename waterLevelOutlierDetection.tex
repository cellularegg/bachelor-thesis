...
\begin{itemize}
    \item List of station addresses \todo{add final stations} \todo{add location description to station list}
\end{itemize}
...
The input data structure for the different approaches is a pandas DataFrame. \todo{add reference?}The structure of the DataFrame is equal for all measurement stations. It contains the following columns:
\begin{itemize}
...
\todo{add examples for TP, FP, TN and FN}
...
In \autoref{table:2386-ch-top-predictions-summary} the top three predictions per model for normalized and not not normalized data is shown. The outlier detection for this station has the worst performance among those tested.
