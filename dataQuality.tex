...
\hl{Calculation errors due to bit flips which is caused by cosmir rays? or int overflow? or because float cannot be 100\% exact?}
...
There are two main methods when it comes to data cleaning or cleansing.\change{"cleaning or cleansing" würde ich hervorheben, weil ich zuerst "data cleaning" und "cleansing" gelesen habe.} Ignoring faulty data or replacing it with a representative value. This paper will use the term data cleaning to describe the process of ignoring or deleting incorrect data and the term data cleansing to portray the process of replacing invalid data with representative values. Faulty, incorrect, invalid or wrong data is data which is inaccurate, incomplete or inconsistent.\\
